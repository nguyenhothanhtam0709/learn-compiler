...
/// @brief Get the position of a new global symbol slot, or die
...
/// @brief Clear all the entries in the
/// local symbol table
...
/// @brief Add a local symbol to the symbol table. Set up its:
/// + type: char, int etc.
/// + structural type: var, function, array etc.
/// + size: number of elements
/// Return the slot number in the symbol table, -1 if a duplicate entry
int addlocl(char *name,
            int type,
            int stype,
            int class,
            int size)
...
/// @brief Given a function's slot number, copy the global parameters
