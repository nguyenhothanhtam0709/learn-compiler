...
/// @brief Parse the current token and return a
...
int parse_type(struct symtable **ctype, int *class)
{
    ...
    case T_LONG:
        ...
        /// @note For the following, if we have a ';' after the
...
/// @brief var_declaration_list: <null>
                                ...
                                int separate_token, int end_token)
{
    ...
    while (Token.token != end_token && Token.token != T_ELLIPSIS)
    {
...
/// @brief function_declaration: type identifier '(' parameter_list ')' ;
...
/// @brief Parse composite type declarations: structs or unions.
...
/// @brief Parse a typedef declaration and return the type
...
/// @brief Given a typedef name, return the type it represents
int type_of_typedef(char *name, struct symtable **ctype)
{
    struct symtable *t;
